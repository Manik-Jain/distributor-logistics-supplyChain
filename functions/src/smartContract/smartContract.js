...
const deploy = async (web3, deployerIndex) => {
    ...
    return escrowContract.deploy({
        ...
        .send(options, function (err, transactionHash) {
            ...
            functions.logger.log('Contract deployment transactionHash: ' + transactionHash);
        })
        .then(function (contract) {
            functions.logger.log('Contract address: ' + contract.options.address);
            return contract;
        }) // chaining
        .catch(function (err) {
            throw new Error(err);
...
//             let txHash = await escrowInstance.methods.deposit(recipient).send({ value: payment, from: deployer });
...
//             txHash = await escrowInstance.methods.withdraw(recipient).send({ from: deployer });
...
// const infura_url = 'https://ropsten.infura.io/v3/<YOUR_INFURA_ROPSTEN_ENDPOINT>';
// // populate this with what is generated from `npm run create-accounts`
// const privateKeys = ['0x...', '0x...', '...'];
...
//         // currying
//         return testContractMethods(newContractInstance)(web3, deployerIndex, recipientIndex);
//     })
//     .catch(err => {
//         console.error(err);
//     }); 
